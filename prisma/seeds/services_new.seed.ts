...
export async function seedServices() {
  ...
  // Получаем группы для связи
  const groups = await prisma.systemServiceGroup.findMany();

  // Услуги
  const services = [
    // Parking and Transport (Group 1)
    ...
    { name: 'Wellness center', systemServiceGroupId: groups[5]?.id || 6, systemServiceTypeId: premiumType?.id || 1 },
  ];
